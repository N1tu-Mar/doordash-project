...
import { totalFeesCents, type OwedBreakdown, type ReceiptMoney } from "../core/money.js";
import { feeLineValues, orderMoneyColumns } from "./order-row.js";
...
export type ServiceReason =
  ...
  /**
   * Recording an OAuth consent row and its encrypted token from the operator
   * CLI, which runs before the user has a Supabase session at all.
   */
  | "oauth_consent_recording"
  /**
...
export async function insertOrder(ctx: UserContext, input: InsertOrderInput): Promise<string> {
  ...
  const money = orderMoneyColumns(input.receipt);
  ...
  const { data, error } = await userDb(ctx).rpc("insert_order_full", {
    ...
    // Money columns come from services/order-row.ts, the single place
    // ReceiptMoney becomes database columns. p_fees_cents is deliberately
    // absent: insert_order_full re-derives it by summing the fee lines it is
    // given, so the scalar and the lines cannot disagree.
    p_subtotal_cents: money.subtotal_cents,
    p_tax_cents: money.tax_cents,
    p_tip_cents: money.tip_cents,
    p_total_cents: money.total_cents,
    // NULL means the receipt did not say which lines were taxable, which is a
    // different fact from "all of them were" (migration 0006).
    p_taxable_base_cents: money.taxable_base_cents,
    p_raw_artifact_path: input.rawArtifactPath,
    p_parser_version: input.parserVersion,
    p_balance_delta_cents: money.balance_delta_cents,
    ...
    // claim recomputed. Labels are defanged inside feeLineValues().
    p_fee_lines: feeLineValues(input.receipt),
  });
