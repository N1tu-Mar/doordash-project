...
`core/money.ts`; `0006` added claims, the taxable base, and the labelling tables.

`pnpm test` applies all six to a real Postgres in-process via PGlite
(`tests/schema-harness.ts`), then runs `supabase/ci/01_assert_schema.sql` against
the result. So the SQL here is executed, not merely read — but see the last
section for what that still does not prove.
...
- **Executed, but never with a user attached.** The migrations apply and the
  catalog assertions pass, so syntax, trigger bodies, policy expressions and view
  definitions are all validated. What is NOT validated is behaviour under a real
  session: no test attempts a cross-tenant read, because that means inventing two
  users and PROMPT.md §2 forbids it. The policies are known to compile and to be
  attached; they are not known to deny. GAPS #5 / S1.
- **PGlite is not the deployment target.** It tracks a different Postgres major
  and runs on wasm32. CI's stock Postgres 16 container is authoritative over it,
  and the real database is authoritative over that.
