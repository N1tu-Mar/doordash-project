...
- **The policies compile and are attached. Nobody has proven they deny.**
  `pnpm test` now applies all six migrations to a real Postgres in-process
  (PGlite) and asserts against the live catalog — `tests/schema.test.ts` for the
  data rules, `tests/schema-security.test.ts` for these. That first execution
  immediately caught `0003` revoking against roles the CI shim never created.

  What those tests cannot do is attempt a cross-tenant read, because doing so
  means inventing two users and PROMPT.md §2 forbids it. So: the SQL parses, the
  policy expressions bind, `is_admin()` is SECURITY DEFINER and stable, and every
  admin view's compiled definition still carries its role predicate. Whether user
  B is actually denied user A's orders is untested and needs two real Supabase
  sessions (GAPS #8). Do not read a green suite as more than that.
